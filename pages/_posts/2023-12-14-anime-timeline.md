...
I have chosen [Lucas numbers]({{ site.url_prefixes.wikipedia }}/Lucas_number) for the sequence, as, in my opinion, they are the most _natural_ numbers one could think of. Lucas numbers are the integers which are closest to integral powers of the [Golden Ratio]({{ site.url_prefixes.wikipedia }}/Golden_ratio) $$\varphi$$, i.e., $$L_n = [\varphi^n]$$ for $$n>1$$. While the presence of $$\varphi$$ in nature is debatable, it pops up as the optimum answer to many problems of nature. Also, it is arguably the most irrational number,[^goldenratio] in the sense that its approximations generated by its continued fraction (below), converge to the $$\varphi$$ _very slowly_.

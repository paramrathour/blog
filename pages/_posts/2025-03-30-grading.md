...
> These add up to $$120$$ marks in total (excluding bonus marks), but grading still works like the usual absolute scheme: $$90+$$ for an AA, $$80+$$ for an AB, $$70+$$ for a BB, and so on.
{: .prompt-tip}
...
| Marks | Letter Grade | Numerical Equivalent
| :-- | :-- | :--
| $$\in(90,120]$$ | AA | $$10$$
| $$\in(80,90]$$ | AB | $$9$$
| $$\in(70,80]$$ | BB | $$8$$
...
- The biggest win here is these $$20$$ extra marks, inspired by [Prof. Shivaram](https://www.cse.iitb.ac.in/~shivaram/)'s Autumn 2021 iteration of [CS747](https://www.cse.iitb.ac.in/~shivaram/teaching/old/cs747-a2021/index.html), which serve as a leverage for students.
...
### The Bigger Problem
Let's zoom out for a second: why are students hyper obsessed with this one-dimensional number in the first place? I think it is due to how highly it is valued by some companies and higher studies programs, which is ultimately the goal of most students. Students feel like there aren't enough 'decent' jobs out there, driving up competition to its limit, but recruiters say genuine skill is in real shortage.
For grading to actually work long-term, it needs to incentivize learning of skills and personality growth over doing anything else. But how do we even do that? Or can we? [Goodhart's Law]({{ site.url_prefixes.wikipedia }}/Goodhart%27s_law) states that when a measure becomes a target, it ceases to be a good measure. So can we really have a system which will quantify and optimise parameters like 'learning' and 'skills'?

Even if we 'fixed' grading by adopting TAG, would that really change anything? Would students suddenly care more about learning? Not every student at IITB is equally into every course. Sometimes they just don't have the time because there are so many courses, or sometimes they're not interested, maybe because the teaching isn't great or they're focused on other courses/projects/hobbies, or sometimes they are just mentally drained from the previous competitive exam preparations that got them here.

I think the way is to look at each student individually, build on their strengths without ignoring their weaknesses. We don't need a one-size-fits-all system, we need plans that _adapt_ to each student. Evaluation needs to improve as a collective effort, particularly now, in these turbulent times with AI, when many traditional skills are cheaply available and are no longer what future careers will require.

## Conclusion
Remember, grades don't necessarily correlate with your end goal, so focus on the goal and not the grades..

Of course, this scheme is not perfect, and the mark distribution is kinda arbitrary[^fix]. However, the goal was to present key ideas, which can inspire you to design an evaluation scheme that best fits your own context :)

## Footnotes
[^fix]: There is a way to optimally choose weightages, again by focussing on leniency, check out [The Grading Polyhedron](https://www.ams.jhu.edu/~grimmer/Grading.pdf) by Benjamin Grimmer.